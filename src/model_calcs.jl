...
if gpu_enabled
  begin
    ...
    angmom_precalc = quote tmp_angmom = calculate_angmoms(model.wfcs,V,centers,cu_dims,Lx,Ly,Lz,n1,n2)::Array{Array{Complex{T},1},2}
  ...
  begin
    ...
    cm_tcalc = quote tmp_cm = calculate_k_dips(model.dip_raw,k) 
    end
    ...
    angmom_cinit = quote 
      eigmom = [[zero(Complex{T}) for n=1:3] for m=1:length(atoms)]
      eigspin = [[zero(Complex{T}) for n=1:3] for m=1:length(atoms)] 
    end
    pot_cinit = :(eigpot = zero(Complex{T}))
    cm_calc = quote eigcm_x += conj(c1)*c2*tmp_cm[i1,i2].x
    ...
    for name in function_names
      ...
      @eval function $(Symbol(name*"_bloch")){T<:AbstractFloat}(model::WannierModel{T},$(func_vars...),k_points::Array{Array{T,1},1})
        ...
        for j=1:size(k_points)[1]
          ...
          construct_bloch_sums(model.wfcs,k_wfcs,k,cu_indices,CuArray(coefficients))
