    ...
    open(filename) do f
        ...
        while true
            ...
            if line == "BEGIN_DATAGRID_3D_UNKNOWN"
                ...
                t          = collect(hcat([read_split_parse(T) for i in 1:3]...)')::Matrix{T}
                supercell  = Mat3{T}(t)
                lines      += 1 + 1 + 3
                break
            end
...
function read_values_from_xsf(::Type{T}, filename::String) where {T<:AbstractFloat}
    open(filename) do f
        while !eof(f)
            line = readline(f)
            if line == "PRIMVEC"
                cell = [Point3{T}.(parse.(T, split(readline(f)))) for i in 1:3]
            end
...
function readhami(chk, eig_file::AbstractString)
    ...
    for (iR, (h, R, shifts, nshifts, d)) in
        enumerate(zip(Hr_t, R_cryst, ws_shifts, ws_nshifts, degens))
        for i in eachindex(h)
            ...
            for is in 1:ns
                ...
                h1 = out[rcryst]
                if h1 === nothing
                    h1 = TbBlock{T,LT,Matrix{Complex{T}}}(rcryst, c * rcryst,
                                                          zeros(ComplexF64, chk.n_wann,
                                                                chk.n_wann),
                                                          zeros(ComplexF64, chk.n_wann,
                                                                chk.n_wann))
                    push!(out, h1)
...
"""
function read_colin_hami(up_chk, down_chk, up_eig_file::AbstractString,
                         down_eig_file::AbstractString)
...
function readhami(job::Job)
    ...
    elseif haskey(wcalc, :spinors) 
        hami = make_noncolin.(readhami(read_chk(chk_files[1]),
                                       joinpath(job, eig_files[1])))
    else
...
function write_dip_file(filename::String, points, cell, atoms, names, direction)
    open(filename, "w") do f
        ...
        write(f, "$(origin[1])   $(origin[2])   $(origin[3])\n")
        write(f,
              "$(points[end,1,1][1][1]-origin[1])   $(points[end,1,1][1][2]-origin[2])   $(points[end,1,1][1][3]-origin[3])\n")
        write(f,
              "$(points[1,end,1][1][1]-origin[1])   $(points[1,end,1][1][2]-origin[2])   $(points[1,end,1][1][3]-origin[3])\n")
        write(f,
              "$(points[1,1,end][1][1]-origin[1])   $(points[1,1,end][1][2]-origin[2])   $(points[1,1,end][1][3]-origin[3])\n")
        for point in points
    ...
    open(filename) do f
        while !eof(f)
            ...
            if line == " DATAGRID_3D_DENSITY"
                ...
                k2 = 1
                while line != "END_DATAGRID_3D"
                    tmp = Array{Complex{T}}(map(x -> (v = tryparse(T, x);
                                                      isnull(v) ?
                                                      Complex(0.0,
                                                              0.0) :
                                                      Complex{T}(get(v),
    ...
    fourier_q_to_R(chk.kpoints, R_cryst) do iR, ik, phase
        for m in 1:nwann
            for n in 1:nwann
                for nn in 1:nntot
                    r_R[iR][n, m] += m == n ?
                                     -wb[nn] .* ustrip.(kbonds[ik][nn].vr) .*
                                     imag(log(m_matrix[n, m, nn, ik])) .* phase :
                                     1im * wb[nn] .* ustrip.(kbonds[ik][nn].vr) .*
                                     m_matrix[n, m, nn, ik] .* phase
                end
            end
        end
    end
